...
namespace EAll4Windows
{
    public partial class Options : Form
    {
        ...
        public bool saving;
        public Options(int deviceNum, string name, EAll4Form rt)
        {
            InitializeComponent();
            ...
            if (filename != "")
            {
                ...
                EAll4Color color = Global.MainColor[device];
                ...
                EAll4Color cColor = Global.ChargingColor[device];
                ...
                EAll4Color fColor = Global.FlashColor[device];
                if (fColor.Equals(new EAll4Color { Red = 0, Green = 0, Blue = 0 }))
                ...
                cBControllerInput.Checked = Global.EAll4Mapping;
            }
            else
            {
                cBFlashType.SelectedIndex = 0;
        ...
        void ControllerReadout_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                int x = Program.rootHub.getEAll4State((int)nUDSixaxis.Value - 1).LX;
                int y = Program.rootHub.getEAll4State((int)nUDSixaxis.Value - 1).LY;
                //else
                ...
                //*/
                x = Program.rootHub.getEAll4State((int)nUDSixaxis.Value - 1).RX;
                y = Program.rootHub.getEAll4State((int)nUDSixaxis.Value - 1).RY;
        ...
        private void InputEAll4(object sender, EventArgs e)
        {
        ...
        public void Set()
        {
            ...
            pnlFull.Location = new Point(pnlFull.Location.X, (cBLightbyBattery.Checked ? (int)(dpix * 42) : (pnlFull.Location.Y + pnlLowBattery.Location.Y) / 2));
            Global.MainColor[device] = new EAll4Color((byte)tBRedBar.Value, (byte)tBGreenBar.Value, (byte)tBBlueBar.Value);
            Global.LowColor[device] = new EAll4Color((byte)tBLowRedBar.Value, (byte)tBLowGreenBar.Value, (byte)tBLowBlueBar.Value);
            Global.ShiftColor[device] = new EAll4Color((byte)tBShiftRedBar.Value, (byte)tBShiftGreenBar.Value, (byte)tBShiftBlueBar.Value);
            Global.ChargingColor[device] = new EAll4Color(btnChargingColor.BackColor);
            Global.FlashType[device] = (byte)cBFlashType.SelectedIndex;
            if (btnFlashColor.BackColor != pBController.BackColor)
                Global.FlashColor[device] = new EAll4Color(btnFlashColor.BackColor);
            else
                Global.FlashColor[device] = new EAll4Color(Color.Black);
            ...
            Global.EAll4Mapping = cBControllerInput.Checked;
        ...
        {
            pBController.Image = Properties.Resources.EAll4_Controller;
        }
        ...
        private void btnFlashColor_Click(object sender, EventArgs e)
        {
            ...
            if (advColorDialog.ShowDialog() == DialogResult.OK)
            {
                if (advColorDialog.Color.GetBrightness() > 0)
                    ...
                    btnFlashColor.BackColor = pBController.BackColor;
                Global.FlashColor[device] = new EAll4Color(advColorDialog.Color);
            }
            if (device < 4)
                EAll4LightBar.forcelight[device] = false;
        }
        ...
        {
            if (Global.FlashColor[device].Equals(new EAll4Color { Red = 0, Green = 0, Blue = 0 }))
            {
                btnFlashColor.BackColor = pBController.BackColor;
            }
        }
