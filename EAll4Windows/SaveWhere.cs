...
namespace EAll4Windows
{
    public partial class SaveWhere : Form
    {
        ...
        private void bnPrgmFolder_Click(object sender, EventArgs e)
        {
            ...
            {
                try { Directory.Delete(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\EAll4Windows", true); }
                catch { }
            }
            else if (!multisaves)
        ...
        private void bnAppdataFolder_Click(object sender, EventArgs e)
        {
            ...
            if (multisaves && !cBDeleteOther.Checked)
                ...
                catch (UnauthorizedAccessException) { MessageBox.Show("Cannot Delete old settings, please manaully delete", "EAll4Windows"); }
            else if (!multisaves)
                Save(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\EAll4Windows\\Profiles.xml");
            Global.SaveWhere(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\EAll4Windows");
            Close();
        }
        ...
        private void SaveWhere_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (String.IsNullOrEmpty(Global.appdatapath))
                if (MessageBox.Show(Properties.Resources.ALocactionNeeded, Properties.Resources.CloseEAll4W,
